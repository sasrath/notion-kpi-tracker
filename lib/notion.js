...
/**
 * Query a Notion database directly via the SDK's databases.query() endpoint.
 * This bypasses the Notion search index so newly-created pages appear immediately.
 * Works both locally and on Vercel since NOTION_API_KEY is always available.
 */
async function queryDatabase(databaseId, filter) {
  const notion = getNotionSDK(); // always use SDK — MCP has no databases.query tool
  const pages = [];
  let startCursor;
  ...
  console.log(`[queryDB] ${databaseId.slice(0,8)}… → ${pages.length} pages`);
  return pages;
}
...
export async function findOrCreateClient(name, industry = null, website = null) {
  // Query by exact name — uses databases.query() so newly-created clients are found immediately
  const pages = await queryDatabase(DB.clients, {
    property: "Name",
    title: { equals: name },
  });
...
// Run async tasks in parallel with a concurrency cap to avoid Notion rate limits
...
export async function saveKPIs({ reportId, clientId, kpis, quarter, year }) {
  const validKPIs = kpis.filter((k) => k.value !== null && k.value !== undefined);

  const settled = await parallelBatch(validKPIs, async (kpi) => {
    const page = await notionCall("API-post-page", {
  ...
  if (errors.length > 0 && results.length === 0) {
    throw new Error(`All KPI saves failed. First error: ${errors[0]}`);
  }
  if (errors.length > 0) {
...
export async function getKPIs({ clientId, quarter, year } = {}) {
  let pages = await queryDatabase(DB.kpis);
...
export async function deleteKPIs(pageIds) {
  const settled = await parallelBatch(pageIds, async (id) => {
    await deleteKPI(id);
    return id;
  });
