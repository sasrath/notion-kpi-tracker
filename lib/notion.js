/**
 * Notion data layer — powered by Notion MCP Server
 *
 * All Notion operations go through the MCP tool calls defined in notion-mcp.js.
 * Write operations use API-post-page. Read operations use API-post-search
 * with client-side filtering by parent database ID.
 */
...
async function searchPagesInDB(databaseId, query = "") {
  ...
  do {
    ...
    const res = await callNotionTool("API-post-search", args);
...
export async function findOrCreateClient(name, industry = null, website = null) {
  ...
  // Create new client via MCP
  const page = await callNotionTool("API-post-page", {
