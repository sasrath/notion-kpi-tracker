...
export function buildForecastChartData(kpis, clients, forecasts) {
  ...
  // Collect all periods (actual + forecast)
  const periodMap = new Map(); // periodLabel → { period, sortKey, ...clientValues }
  for (const k of revenueKPIs) {
    const name = clientMap[k.clientId] ?? "Unknown";
  ...
  for (const fc of forecasts) {
    if (!allClients.includes(fc.client)) continue;
    const period = `${fc.nextQuarter} ${fc.nextYear} (F)`;
    const sortKey = fc.nextYear * 10 + (QUARTER_ORDER[fc.nextQuarter] ?? 0);
    if (!periodMap.has(period)) periodMap.set(period, { period, sortKey, _forecast: true });
    periodMap.get(period)[fc.client] = fc.forecastValue;
  }

  const chartData = [...periodMap.values()].sort((a, b) => a.sortKey - b.sortKey);
...
export function getKPIDistribution(kpis) {
  const counts = {};
  for (const kpi of kpis) {
    counts[kpi.name] = (counts[kpi.name] || 0) + 1;
  }
