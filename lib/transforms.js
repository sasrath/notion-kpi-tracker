...
export function buildForecastChartData(kpis, clients, forecasts) {
  ...
  actuals = actuals.slice(-3); // last 3 quarters only

  // Add the single forecast period (fuzzy-match AI client name to actual client name)
  ...
  for (const fc of forecasts) {
    ...
    }) ?? (allClients.length === 1 ? allClients[0] : null);
