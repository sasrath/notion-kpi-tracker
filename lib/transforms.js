...
const SUMMARY_KPIS = [
  { key: "Revenue",        match: (n) => /^(total\s+)?(net\s+)?revenue$/i.test(n) || /^net\s+sales$/i.test(n) || /^total\s+net\s+sales$/i.test(n) },
...
export function getRevenueTrend(kpis, clients) {
  ...
  // Match only the top-line "Revenue" KPI (same rule as the forecast) so we
  // don't accidentally pick up segment revenues (CCG Revenue, DCAI Revenue…)
  // which share the same period but are much smaller numbers.
  const revenueKPIs = kpis.filter((k) => matchSummaryKPI(k.name) === "Revenue");
...
// ─── GET SEGMENT REVENUE TREND ───────────────────────────────────
// Returns bar chart data for all named segment revenue KPIs
// (anything matching "<Name> Revenue" that is NOT the top-line "Revenue").

export function getSegmentRevenueTrend(kpis, clients) {
  const clientMap = Object.fromEntries(clients.map((c) => [c.id, c.name]));

  // Match segment revenues: name ends with " Revenue" but is NOT the bare top-line
  const segmentKPIs = kpis.filter((k) => {
    const n = k.name.trim();
    return /\bRevenue$/i.test(n) && !/^Revenue$/i.test(n) && !/ \(Non-GAAP\)$/i.test(n);
  });

  if (segmentKPIs.length === 0) return { chartData: [], segmentNames: [] };

  // Build sorted period list
  const periodSet = new Map();
  for (const k of segmentKPIs) {
    const label = `${k.quarter} ${k.year}`;
    const sortKey = k.year * 10 + (QUARTER_ORDER[k.quarter] ?? 0);
    if (!periodSet.has(label)) periodSet.set(label, sortKey);
  }
  const periods = [...periodSet.entries()]
    .sort((a, b) => a[1] - b[1])
    .map(([label]) => label);

  // Collect unique segment names. Prefix with client name when multiple clients.
  const clientIds = [...new Set(segmentKPIs.map((k) => k.clientId))];
  const multiClient = clientIds.length > 1;

  const segmentSet = new Set();
  for (const k of segmentKPIs) {
    const clientName = clientMap[k.clientId] ?? "Unknown";
    const segLabel = multiClient ? `${clientName} — ${k.name}` : k.name;
    segmentSet.add(segLabel);
  }
  const segmentNames = [...segmentSet];

  // Build recharts-compatible rows: { period, "CCG Revenue": 7700, "DCAI Revenue": 3800, … }
  const chartData = periods.map((period) => {
    const [quarter, yearStr] = period.split(" ");
    const year = Number(yearStr);
    const row = { period };
    for (const k of segmentKPIs) {
      if (k.quarter !== quarter || k.year !== year) continue;
      const clientName = clientMap[k.clientId] ?? "Unknown";
      const segLabel = multiClient ? `${clientName} — ${k.name}` : k.name;
      row[segLabel] = k.value;
    }
    return row;
  });

  return { chartData, segmentNames };
}
...
export function formatValue(value, unit) {
  ...
  switch (unit) {
    case "$M":
      // Show in Billions if >= 1000M
      if (Math.abs(value) >= 1000) {
        return `$${(value / 1000).toLocaleString("en-US", { maximumFractionDigits: 2 })}B`;
      }
...
// Format $M values for chart tooltips — auto-scales to $B when >= 1000M
export function formatDollarM(value) {
