...
// ─── SECTION 11: JUDGES AUTH ───────────────────────────────────
console.log("\n── 11. Judges Authentication ────────────────────────────────");

await test("Judges auth: rejects wrong password", async () => {
  const { status } = await postJSON("/api/judges-auth", {
...
await test("Judges auth: rejects empty password", async () => {
  const { status } = await postJSON("/api/judges-auth", { password: "" });
  if (status !== 401) throw new Error(`Expected 401, got ${status}`);
  return "401 returned";
});

await test("Judges auth: accepts correct password", async () => {
  const { status, data } = await postJSON("/api/judges-auth", {
    password: "NotionChallenge2026!",
  });
...
await test("GET /judges returns 200", async () => {
