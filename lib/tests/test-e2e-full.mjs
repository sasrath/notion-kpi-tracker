...
// ─── SECTION 19: DATA TRANSFORM VALIDATION ─────────────────────
console.log("\n── 19. Data Transform Validation ────────────────────────────");

// Import transforms locally for pure-JS tests
const { formatDollarM, formatValue, getLatestSummary, deduplicateKPIs } = await import("../../lib/transforms.js");

// formatDollarM tests
await test("formatDollarM: null → '—'", () => {
  const r = formatDollarM(null);
  if (r !== "—") throw new Error(`Got ${r}`);
  return r;
});

await test("formatDollarM: 500 → '$500M'", () => {
  const r = formatDollarM(500);
  if (r !== "$500M") throw new Error(`Got ${r}`);
  return r;
});

await test("formatDollarM: 1000 → shows $B", () => {
  const r = formatDollarM(1000);
  if (!r.endsWith("B")) throw new Error(`Expected $B, got ${r}`);
  return r;
});

await test("formatDollarM: 12750 → '$12.75B'", () => {
  const r = formatDollarM(12750);
  if (r !== "$12.75B") throw new Error(`Got ${r}`);
  return r;
});

await test("formatDollarM: 95359 → shows $B", () => {
  const r = formatDollarM(95359);
  if (!r.endsWith("B")) throw new Error(`Expected $...B, got ${r}`);
  return r;
});

// formatValue $M auto-scale
await test("formatValue: $M 500 → in $M", () => {
  const r = formatValue(500, "$M");
  if (!r.endsWith("M")) throw new Error(`Expected $...M, got ${r}`);
  return r;
});

await test("formatValue: $M 1500 → in $B", () => {
  const r = formatValue(1500, "$M");
  if (!r.endsWith("B")) throw new Error(`Expected $...B, got ${r}`);
  return r;
});

await test("formatValue: % 46.9 → '46.9%'", () => {
  const r = formatValue(46.9, "%");
  if (r !== "46.9%") throw new Error(`Got ${r}`);
  return r;
});

// Revenue matcher — broadened SUMMARY_KPIS
const makeKPI = (name) => ({ id: name, name, clientId: "t1", value: 5000, unit: "$M", quarter: "Q1", year: 2025, confidence: "High", source: "Test" });
const matchClients = [{ id: "t1", name: "TestCo" }];

await test("Revenue matcher: 'Revenue' matches top-line", () => {
  const summary = getLatestSummary([makeKPI("Revenue")], matchClients, "t1");
  if (!summary.some(s => s.name === "Revenue" && s.value != null)) throw new Error(`Got: ${summary.map(s => `${s.name}=${s.value}`).join(", ")}`);
  return "matched";
});

await test("Revenue matcher: 'Total Revenue' matches", () => {
  const summary = getLatestSummary([makeKPI("Total Revenue")], matchClients, "t1");
  if (!summary.some(s => s.name === "Revenue" && s.value != null)) throw new Error(`Not matched. Got: ${summary.map(s => `${s.name}=${s.value}`).join(", ")}`);
  return "matched";
});

await test("Revenue matcher: 'Net Revenue' matches", () => {
  const summary = getLatestSummary([makeKPI("Net Revenue")], matchClients, "t1");
  if (!summary.some(s => s.name === "Revenue" && s.value != null)) throw new Error(`Not matched. Got: ${summary.map(s => `${s.name}=${s.value}`).join(", ")}`);
  return "matched";
});

await test("Revenue matcher: 'Net Sales' matches", () => {
  const summary = getLatestSummary([makeKPI("Net Sales")], matchClients, "t1");
  if (!summary.some(s => s.name === "Revenue" && s.value != null)) throw new Error(`Not matched. Got: ${summary.map(s => `${s.name}=${s.value}`).join(", ")}`);
  return "matched";
});

await test("Revenue matcher: 'Total Net Sales' matches", () => {
  const summary = getLatestSummary([makeKPI("Total Net Sales")], matchClients, "t1");
  if (!summary.some(s => s.name === "Revenue" && s.value != null)) throw new Error(`Not matched. Got: ${summary.map(s => `${s.name}=${s.value}`).join(", ")}`);
  return "matched";
});

await test("Revenue matcher: 'CCG Revenue' does NOT match top-line", () => {
  const summary = getLatestSummary([makeKPI("CCG Revenue")], matchClients, "t1");
  if (summary.some(s => s.name === "Revenue" && s.value != null)) throw new Error("CCG Revenue incorrectly matched as top-line Revenue");
  return "correctly excluded";
});

// Confirm route response shape — verify error responses have 'error' field
// and valid success responses include 'kpiCount' (verified by the full save/delete cycle in Section 13)
await test("Confirm route: 400 response has 'error' field", async () => {
  const res = await fetch(BASE + "/api/ingest/confirm", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ clientName: "ShapeTest" }), // missing kpis
  });
  const data = await res.json();
  if (res.status !== 400) throw new Error(`Expected 400, got ${res.status}`);
  if (!data.error) throw new Error("Error response missing 'error' field");
  return `error field: "${data.error}"`;
});

await test("Confirm route: valid request returns kpiCount", async () => {
  // Save 1 KPI under existing Apple client, then immediately clean up
  const res = await fetch(BASE + "/api/ingest/confirm", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      ...
      quarter: "Q1",
      year: 2019,
      url: "https://example.com/e2e-tc76",
      kpis: [{ name: "E2E TC76 Probe", value: 1, unit: "$M", source: "Test" }],
    }),
  });
  const data = await res.json();
  if (!res.ok) return `Notion not connected (expected in CI): ${data.error?.slice(0, 60)}`;
  if (typeof data.kpiCount !== "number") throw new Error(`kpiCount missing: ${JSON.stringify(data)}`);
  // Clean up: delete the test KPI immediately
  const kpiRes = await fetch(BASE + "/api/kpis");
  const kpiData = await kpiRes.json();
  const probe = kpiData.kpis?.find((k) => k.name === "E2E TC76 Probe" && k.year === 2019);
  if (probe) {
    await fetch(BASE + "/api/kpis/delete", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ids: [probe.id] }),
    });
  }
  return `kpiCount=${data.kpiCount}, probe ${probe ? "cleaned up" : "not found in index yet"}`;
});
